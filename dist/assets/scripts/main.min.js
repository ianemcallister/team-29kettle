...
function adminWeeklyScheduleController(e,l,o){console.log("in the adminWeeklyScheduleController  controller ",l),Object.keys(l)<1&&(o="admin/schedule/weekly?Yr="+o().format("YY")+"&&Wk="+o().week(),console.log(o),e.url(o))}ckc.controller("adminWeeklyScheduleController",adminWeeklyScheduleController),adminWeeklyScheduleController.$inject=["$location","$routeParams","moment"];
...
function Database(a,t){return{get:{channelEngagments:function(e){var n=a.defer();return t({method:"GET",url:"/db/engagments?channelId="+e}).then(function(e){n.resolve(e.data)},function(e){console.log("Error: ",e)}),n.promise}}}}ckc.factory("Database",Database),Database.$inject=["$q","$http"];
